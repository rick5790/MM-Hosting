(function () {
  ...
  function tickWeeklyCountdown() {
    const el = document.querySelector(".weekly-countdown[data-weekly-deadline]");
    if (el) {
      el.textContent = formatWeeklyCountdown(
  ...
  function renderWeeklyOrderEditor() {
    ...
    return `
      <details class="section-block weekly-editor-card collapsible-section weekly-editor-fold">
        <summary class="collapsible-summary weekly-editor-summary">
          <div>
            <p class="eyebrow">Weekly Group</p>
            <h2 class="section-heading">编辑本次团购</h2>
            <p class="weekly-editor-desc">设置本次团购的标题、时间和开放状态。</p>
          </div>
          <div class="weekly-editor-fold-meta">
            <span class="weekly-status-pill ${(weeklyOrder.is_open || extraWindowActive) ? "is-open" : ""}"><span class="weekly-status-dot"></span>${extraWindowActive ? "加单窗口开放中" : (weeklyOrder.is_open ? "开放中" : "未开放")}</span>
            <span class="weekly-countdown" data-weekly-start="${escapeHtml(currentStartAt)}" data-weekly-deadline="${escapeHtml(currentDeadlineAt)}">${escapeHtml(formatWeeklyCountdown(currentStartAt, currentDeadlineAt))}</span>
          </div>
        </summary>
        <form class="weekly-editor-form" id="weeklyOrderForm">
          <div class="weekly-fields-grid">
            ...
            <label class="field">
              ...
              <small class="field-hint">控制用户端页面显示状态。</small>
            </label>
      ...
      <details class="weekly-create-card">
          <summary class="weekly-create-summary">
            <span class="weekly-create-icon">+</span>
            <div class="weekly-create-text">
              <div class="weekly-create-title">创建新一期团购</div>
              <div class="weekly-create-sub">在当前团购基础上新建，编号自动 +1</div>
            </div>
            <span class="weekly-create-next">下一期 · ${escapeHtml(nextTitle)}</span>
          </summary>
          <form class="form-stack" id="weeklyOrderCreateForm">
            <div class="weekly-fields-grid">
              <label class="field">
                <span>团购标题</span>
                <input class="input" type="text" name="title" value="${escapeHtml(nextTitle)}" required>
              </label>
              <label class="field">
                <span>开放状态</span>
                <select class="select" name="is_open">
