...
// 允许携带 cookie 的跨域请求：credentials 模式不能用 origin: "*"，
// 必须白名单。没有 Origin 头的请求（curl、同源管理端）直接放行。
const ALLOWED_ORIGINS = [
  "https://makkiemua.com",
  "https://www.makkiemua.com",
  "http://localhost:8000",
  "http://127.0.0.1:8000",
  "http://localhost:3000",
  "http://localhost:5173"
...
// 从根路径也服务 admin UI 静态文件（index.html 用相对路径 ./admin.js / ./admin.css）。
// static 找不到文件会 next()，所以 /api/* 仍走后面的路由。
...
// 规格要求的顶层别名：GET /api/me、POST /api/logout（与 /api/auth/me、/api/auth/logout 等价）
const authController = require("./controllers/authController");
const { requireLogin } = require("./middleware/auth");
app.get("/api/me", requireLogin, authController.me);
