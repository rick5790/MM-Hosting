(function () {
  ...
  function renderCakeOrderDetail() {
    ...
    return `
      ...
      ${order.status === "cancelled" && (order.cancel_reason || order.cancelled_at) ? `<div class="section-block"><p class="section-title">取消记录</p><div>${escapeHtml(order.cancel_reason || "未记录原因")}</div>${order.cancelled_at ? `<div class="cancelled-at">取消时间：${escapeHtml(formatDate(order.cancelled_at))}</div>` : ""}</div>` : ""}
  ...
  function renderInlineOrderExpanded(order) {
    return `
      <div class="order-inline-body">
        ...
        ` : ""}
        ${order.status === "cancelled" && (order.cancel_reason || order.cancelled_at) ? `
          <div class="order-inline-section">
            <span class="order-inline-label">取消记录</span>
            <div class="order-inline-note">${escapeHtml(order.cancel_reason || "未记录原因")}</div>
            ${order.cancelled_at ? `<div class="cancelled-at">取消时间：${escapeHtml(formatDate(order.cancelled_at))}</div>` : ""}
          </div>
        ` : ""}
    ...
    return `
      ...
      <div class="detail-grid">
        ...
        ${order.status === "cancelled" && (order.cancel_reason || order.cancelled_at) ? `
          <div class="detail-meta">
            <span>取消原因</span>
            <strong>${escapeHtml(order.cancel_reason || "未记录原因")}</strong>
          </div>
          ${order.cancelled_at ? `
            <div class="detail-meta">
              <span>取消时间</span>
              <strong>${escapeHtml(formatDate(order.cancelled_at))}</strong>
            </div>` : ""}` : ""}
        ${Number(order.deposit_applied) > 0 ? `
          <div class="detail-meta">
            <span>订单原价</span>
        ...
        <div class="analytics-filter-card">
          <div class="analytics-filter-grid">
            <label class="field"><span>时间段</span><select class="select" id="analyticsRange">
              ...
              <option value="month" ${state.filters.analytics_range === "month" ? "selected" : ""}>本月</option>
            ...
            <label class="field analytics-date-field"><span>开始日期</span><span class="analytics-date-control"><input class="analytics-date-input" id="analyticsStart" type="date" value="${escapeHtml(bounds.start)}"></span></label>
            <label class="field analytics-date-field"><span>结束日期</span><span class="analytics-date-control"><input class="analytics-date-input" id="analyticsEnd" type="date" value="${escapeHtml(bounds.end)}"></span></label>
          </div>
